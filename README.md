...
"rubyish", it also requires the ROF gem.  The deploy then compiles the Go
code, which is in a subsirectory, and installs that.  Finally the puppet
scripts run and setup the environment, and install the config files (e.g.
start the Go service at startup).

The Go service will monitor a directory for new files.  It then spawns a
ROF process to handle the new file.  In this way most of the processing
